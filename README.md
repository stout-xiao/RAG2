...
1. 安装依赖（示例）：
   ...
   ```
2. 直接运行（若无本地模型，第一次运行会直接安装）
  ```bash
   python main.py
  ```
3. （可选）准备本地模型（离线环境请提前下载并将路径写入 `EMBED_MODEL`、`NLI_MODEL`）。
4. 配置云端 LLM 访问。
...
```
默认会生成 `faiss.index` 与 `faiss_meta.json`。

## 逻辑要点
- 切片：Hotpot 每 4 句重叠 1 句；保留 `doc_title`/`chunk_id`。
